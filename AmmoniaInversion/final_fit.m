...
b = [23694.4;23722.6;23870.1;
    ...
    22834.1;18178.0];
% u is the measurement uncertainty, w is the weighted matrix.
u = [0.9,1.1,1.0,0.9,0.8,1.2,0.9,0.9,1.0,0.8,1.0]
w = diag(1./u.^2,0);% calculate weighted matrix
M = inv(transpose(A)*w*A); % diagonal values are uncertainties
% calculate constants with weighted matrix
x = inv(transpose(A)*w*A)*transpose(A)*w*b
uncert = sqrt(diag(M)) % uncertainties in constant values vo,c1,c2,c3,c4,c5
% OUTPUT
%  x =
% 
%    1.0e+04 *
